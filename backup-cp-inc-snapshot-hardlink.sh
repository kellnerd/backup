...
do
	...
	# -u update files only (do not remove and re-copy files, this destroys the hard links!), without deletion, alternative:
	# log rsync -avhh --delete "$sourcePath/$dir" "$backupDir"
done
# 4.9GiB: cp 42,941s / rsync 50,974s/44,782s

# create the new snapshot using hard links
logtime "Creating snapshot '$snapshotDir'"
